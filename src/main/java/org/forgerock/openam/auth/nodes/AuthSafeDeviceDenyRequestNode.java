 ...
 * headers.
 */
@Node.Metadata(outcomeProvider = SingleOutcomeNode.OutcomeProvider.class, configClass = AuthSafeDeviceDenyRequestNode.Config.class)
public class AuthSafeDeviceDenyRequestNode extends SingleOutcomeNode {

	private static final Logger logger = LoggerFactory.getLogger(AuthSafeDeviceDenyRequestNode.class);
	...
	@Inject
    public AuthSafeDeviceDenyRequestNode(@Assisted Config config) {
        this.config = config;
    }
    ...
    public Action process(TreeContext context) {
        ...
        HttpRequest request = HttpRequest.newBuilder()
        		.uri(URI.create(String.format("https://a.authsafe.ai/v1/devices/%1$s/deny", sharedState.get("responseDeviceId").asString()))).POST(BodyPublishers.ofString(""))
